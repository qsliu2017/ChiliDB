...
impl Ctid {
    /// Width of the canonical execution-interchange encoding, independent of Rust's
    /// layout, PostgreSQL's ABI, and on-disk page formats. Planning carries it in
    /// the internal non-nullable `FixedSizeBinary(6)` column `__ctid`.
